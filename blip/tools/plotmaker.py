...
def cornermaker(post, params,parameters, inj, Model, Injection=None, split_by=None, saveto=None, histcolor='darkmagenta'):
    ...
    for i, parameter_subset_i in enumerate(parameter_subsets):
        ...
        for ii in range(npar):
            ...
            hist_ys, edges = np.histogram(post_ii, bins=bins_ii,density=False)
            edge_centers = 0.5 * (edges[:-1] + edges[1:])
            interpolator = interp1d(edge_centers, hist_ys, kind='nearest')
...
if __name__ == '__main__':
    ...
    ## set the matplotlib style setting
    matplotlib.rcParams.update(matplotlib.rcParamsDefault)
